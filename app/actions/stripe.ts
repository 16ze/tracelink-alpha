...
/**
 * Action serveur pour créer une session de checkout Stripe pour le plan Starter
 * 
 * Cette fonction retourne un objet avec `url` en cas de succès ou `error` en cas d'échec.
 * La redirection est gérée côté client pour éviter les problèmes avec redirect().
 * 
...
export async function createStarterCheckoutSession(
  ...
  if (!starterPriceId) {
    console.error("❌ STRIPE_STARTER_PRICE_ID n'est pas définie");
    return { error: "Configuration Stripe incomplète pour le plan Starter" };
  }
  ...
  try {
    ...
    // 🛒 CRITIQUE: Log avant création de la session pour traçabilité
    console.log('🛒 Création session Starter pour brand:', brandId);
    console.log('   👤 User ID:', user.id);
    console.log('   📦 Metadata qui sera envoyée:', {
      brand_id: brandId,
      user_id: user.id,
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        brand_id: brandId, // C'est la clé vitale
        user_id: user.id,
        plan: "starter", // Indication du plan pour le webhook
      },
      locale: locale === "en" ? "en" : "fr",
    });

    console.log("✅ Session de checkout Starter créée avec succès:", session.id);
    ...
    // Logs détaillés pour identifier la vraie erreur
    console.error("❌ STRIPE ERROR DETAILS (Starter):", error);
    if (error instanceof Error) {
      console.error("Message:", error.message);
      console.error("Stack:", error.stack);
      return { error: error.message };
